...
public class AdminController {
	...
	public String SendMessage(Model model, @ModelAttribute("message") String message,
		...
		Mail mail = new Mail();
		mail.setMailFrom("namnguyen2482k1@gmail.com");
...
//			int category_selected = 0;
	...
	// My Category
	@GetMapping("dashboard-mycategories")
	public String DashboardMyCategoryView(Model model) {
		User admin = (User) session.getAttribute("admin");
		if (admin == null) {
			return "redirect:/signin-admin";
		} else {
			List<Category> listCategories = categoryService.findAll();		
			model.addAttribute("listCategories", listCategories);
			return "dashboard-mycategories";
		}
	}

	@GetMapping("/dashboard-mycategories/edit/{id}")
	public String DashboardMyCategoryEditView(@PathVariable int id, Model model) {
		...
		if (admin == null) {
			...
			return "dashboard-my-categories-edit";
		}
	}

	@PostMapping("/dashboard-mycategories/edit")
	public String DashboardMyCategoryEditHandel(Model model, @ModelAttribute("category_id") int category_id,
		...
		if (admin == null) {
			return "redirect:/signin-admin";
				...
				return "redirect:/dashboard-mycategories/edit/" + category_id;
			}
	
	@GetMapping("/delete-category/{id}")
	public String DeleteCategory(@PathVariable int id, Model model, HttpServletRequest request) throws Exception {
		User admin = (User) session.getAttribute("admin");
		if (admin == null) {
			return "redirect:/signin-admin";
		} else {
...
// End 	
	...
	public String DashboardAddCategoryHandel(Model model, @ModelAttribute("category_name") String category_name){
		User admin = (User) session.getAttribute("admin");
		if (admin == null) {
			return "redirect:/signin-admin";
		} else {		
